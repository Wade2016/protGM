...
def make_test_graph():
    ...
    edge_potentials[(0,1)] = np.array([[ 1.21435952,  0.78865583,  5.93958029,  0.91751033,  3.57118778],
                                       [ 1.2537096 ,  0.68910615,  1.69770695,  0.93913777,  0.93399517],
    ...
    for n in graph_name.nodes_iter():
        graph_name.node[n]['node_state_indices'] = [i for i in xrange(len(graph_name.node[n]['node_potential']))]

    return graph_name

# make graph from the example at the end of hetu and chris's paper: http://www.ncbi.nlm.nih.gov/pubmed/21120864
# the grap has three nodes and thre edges
...
# make random chain graph: n_0---n_1---...---n_N; states per node vary from 2 to 5
...
def make_random_chain_or_loop_graph(N,loop):
    ...
    # add indices of node state -- seems like there should be a slich way to avoid this but can't figure it out
    for n in graph_name.nodes_iter():
        graph_name.node[n]['node_state_indices'] = [i for i in xrange(node_state_sizes[n])]
    ...
    for i,edge in enumerate(graph_name.edges_iter()):
        n_a, n_b = edge
        edge_potential = np.random.exponential(1,[len(graph_name.node[n_a]['node_potential']),len(graph_name.node[n_b]['node_potential'])])
...
def make_random_binomial_graph(N_nodes,p_edge):
    ...
    node_state_sizes = np.random.randint(10, high=20, size=graph_name.number_of_nodes())
...
def make_random_tree_graph(N_nodes,gamma_powerlaw=3,N_tries=1000):
    ...
    # initialize and add nodes
    graph_name = nx.random_powerlaw_tree(N_nodes,gamma=gamma_powerlaw,tries=N_tries)
    node_state_sizes = np.random.randint(2, high=5, size=graph_name.number_of_nodes())
...
def make_random_lobster(N_nodes, p1=0.5, p2=0.5, seed=None):
